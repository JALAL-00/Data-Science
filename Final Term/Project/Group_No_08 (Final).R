...
ds<-read.csv("/Users/moontasirmahatbjalal/Desktop/DS FINAL PRO/Practice/winequality-red.csv",header=TRUE,sep=",")
ds
str(ds)
breaks<-c(0, 3, 5, 7, 10)
...
for (attr in numeric_attributes) {
  p <- ggplot(ds, aes(x = !!sym(attr), y = 1)) +
    geom_rug(color = "blue") +
    labs(x = attr, y = "") +
    ggtitle(paste("Rug Plot of", attr))
    print(p)
}
